...
class USAD(nn.Module):
    def __init__(self, epochs, w_size, z_size):
        super().__init__()
        self.epochs = epochs
    ...
    def fit(self, train_loader, val_loader, opt_func=torch.optim.Adam):
        history = []
        optimizer1 = opt_func(
    ...
    def predict(self, test_loader, alpha=0.5, beta=0.5):
        results = []
        with torch.no_grad():
    ...
    def evaluate(self, val_loader, n):
        outputs = [
            self.validation_step(to_device(batch, device()), n) for [batch] in val_loader
        ]
