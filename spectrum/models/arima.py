...
class ARIMA(BaseModel):
    ...
    def fit(self):
        ...
        if self.order is None:
            if len(data) == 0:
                self.order = (1, 1, 1)
                return
            search_data = data[-min(1000, len(data)):]
            best_order, _ = find_optimal_arima_params(
                search_data,
                max_p=self.max_p,
                max_q=self.max_q,
                max_d=self.max_d,
    ...
    def predict(self, values: pl.Series) -> pl.Series:
        data = values.to_numpy()
        if self.order is None:
            # 如果还没调用 fit，就使用一个默认的简单阶数
            self.order = (1, 1, 1)
        scores = compute_anomaly_scores_arima(
            data,
