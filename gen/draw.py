...
def plot_multivariate_timeseries_combined(train_df: pl.DataFrame, test_df: pl.DataFrame, 
                                        dataset_name: str, figsize=(14, 6)):
    """
    绘制训练集和测试集的时间序列数据，每个维度一张图
    训练集和测试集合并在同一张图上，用虚线分隔，并标记测试集中的异常点
    ...
    # 创建输出目录
    output_dir = f"../figures/datasets/{dataset_name}/combined"
    ...
    for i, col in enumerate(value_cols):
        ...
        # 用虚线标记分割点
        ax.axvline(x=split_point, color='gray', linestyle='--', linewidth=1, alpha=0.7)
        ax.text(split_point, ax.get_ylim()[1], ' Train/Test Split ', 
        ...
        if has_label:
            ...
            if np.sum(test_anomaly_mask) > 0:
                # 调整异常点索引以匹配整体坐标系
                anomaly_indices = test_indices[test_anomaly_mask]
                anomaly_values = test_values[test_anomaly_mask]
        ...
        # 设置标题和轴标签
        title = f"{dataset_name} Dataset - Dimension {col.split('_')[1]}"
        ...
        # 保存图形
        filename = f"{dataset_name}-{col.split('_')[1]}-combined.png"
        fig.savefig(os.path.join(output_dir, filename), dpi=150, bbox_inches="tight")
        plt.close(fig)
        
        logging.info(f"Saved combined plot for {dataset_name} dimension {col.split('_')[1]} as {filename}")
...
def plot_multivariate_timeseries_separate(train_df: pl.DataFrame, test_df: pl.DataFrame, 
    ...
    combined_output_dir = f"../figures/datasets/{dataset_name}/combined"
    ...
    for i, col in enumerate(value_cols):
        ...
        # 保存合并图形
        filename = f"{dataset_name}-{col.split('_')[1]}-combined.png"
        fig.savefig(os.path.join(combined_output_dir, filename), dpi=150, bbox_inches="tight")
        plt.close(fig)
        
        logging.info(f"Saved combined plot for {dataset_name} dimension {col.split('_')[1]} as {filename}")
def plot_npy_dataset_combined(train_data: np.ndarray, test_data: np.ndarray, 
                            dataset_name: str, figsize=(14, 6)):
    """
    绘制.npy格式训练集和测试集的时间序列数据，每个维度一张图
    训练集和测试集合并在同一张图上，用虚线分隔，并标记测试集中的异常点
    ...
    split_point = len(train_data)  # 分割点
    ...
    for dim in range(min(n_dims, train_data.shape[1])):
        ...
        if np.sum(test_anomaly_mask) > 0:
            # 调整异常点索引以匹配整体坐标系
            anomaly_indices = test_indices[test_anomaly_mask]
            anomaly_values = test_data[test_anomaly_mask, dim]
        ...
        # 设置标题和轴标签
        title = f"{dataset_name} Dataset - Dimension {dim}"
        ax.set_title(title, fontsize=13, fontweight="bold", pad=10)
        ax.set_xlabel("Data Point Index", fontsize=12, fontweight="bold")
        ax.set_ylabel("Value", fontsize=12, fontweight="bold")
        ...
        # 保存图形
        filename = f"{dataset_name}-{dim}-combined.png"
        fig.savefig(os.path.join(output_dir, filename), dpi=150, bbox_inches="tight")
        plt.close(fig)
        
        logging.info(f"Saved combined plot for {dataset_name} dimension {dim} as {filename}")
...
def visualize_psm_dataset():
    ...
    plot_multivariate_timeseries_combined(train_df, test_df, dataset_name)
...
def visualize_npy_dataset(dataset_name):
    ...
    plot_npy_dataset_combined(train_data, test_data, dataset_name)
